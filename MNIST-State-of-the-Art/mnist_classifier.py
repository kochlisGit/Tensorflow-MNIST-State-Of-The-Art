...
train_inputs = np.reshape( train_inputs, (train_inputs.shape[0], rows, cols, 1) ) / 255.0
test_inputs = np.reshape( test_inputs, (test_inputs.shape[0], rows, cols, 1) ) / 255.0
...
checkpoint = ModelCheckpoint(filepath='weights/mnist_cnn_callback.h5',
                             monitor='val_loss',
                             mode='min',
                             save_best_only=True,
                             verbose=1)
...
# Saving the History.
pickle_file = open('weights/MNIST_history.pickle', 'wb')
